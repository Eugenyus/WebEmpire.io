...
export default function RoadmapStepModal({ isOpen, onClose, onSave, loading, editData = null }) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (editData) {
        // Fetch checklist items and quiz items for this roadmap task
        const fetchItems = async () => {
          ...
          const newFormData = {
            ...
            videoLinks: editData.video_links?.length > 0 
              ? editData.video_links.map(v => v.url)
              : [''],
                ...
                <div className={errors.description ? 'error' : ''}>
                  <CKEditor
                    ...
                    config={{
                      ...
                      removePlugins: ['MediaEmbed', 'Table', 'TableToolbar', 'CKFinderUploadAdapter', 'CKFinder', 'EasyImage', 'Image', 'ImageCaption', 'ImageStyle', 'ImageToolbar', 'ImageUpload'],
                    }}
                  />
                </div>
                {errors.description && (
