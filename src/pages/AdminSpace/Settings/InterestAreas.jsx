...
export default function InterestAreas() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ id: '', title: '', description: '', stripe_price_id: '', price: '0' });
      setIsEditing(false);
      setEditingArea(null);
  ...
  const handleEdit = (area) => {
    setFormData({
      ...
      stripe_price_id: area.stripe_price_id || '',
      price: area.price?.toString() || '0'
    });
    setIsEditing(true);
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg border">
        <div className="grid grid-cols-[1fr,2fr,1fr,1fr,auto] gap-4 p-4 border-b bg-gray-50 font-medium">
          <div>Title</div>
          <div>Description</div>
          <div>Price</div>
        ...
        {interestAreas.map((area) => (
          <div key={area.id} className="grid grid-cols-[1fr,2fr,1fr,1fr,auto] gap-4 p-4 border-b last:border-0">
            <div>{area.title}</div>
            <div className="text-gray-600">{area.description}</div>
            <div className="text-gray-600">{formatCurrency(area.price)}</div>
            <div className="text-gray-600">{area.stripe_price_id || '-'}</div>
