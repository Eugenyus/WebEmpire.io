...
CREATE OR REPLACE FUNCTION public.reorder_roadmap_tasks(
    task_id uuid,
    new_order integer,
    area_id text
)
...
DROP POLICY IF EXISTS "Allow reorder for admin users" ON public.roadmap_tasks;
...
GRANT EXECUTE ON FUNCTION public.reorder_roadmap_tasks(uuid, integer, text) TO authenticated;